...
export const parseMarkup = async ({ markup, middleware = {}, mltype, convertMarkup = true }) => {
  ...
  if (!mltype) {
    const mlMatch = markup.match(/(?:odom-ml)=["']?((?:.(?!["'`]?\s+(?:\S+)=|\s*\/?[>"']))+.)["']?/);
  ...
  if (custom) {
    ...
    dom = await (parser ? parser(markup, mltype) : parse({ markup, type: mltype, root: tagName, convertMarkup }));
...
const parse = async ({ markup, type, root, converter, convertMarkup }) => {
  ...
  return root ? doc.querySelector(root) : doc.querySelector("body").firstElementChild;
};
